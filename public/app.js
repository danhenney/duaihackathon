...
function assetOverviewText(symbol) {
  const map = {
    ...
    NBIS: "Nebius Group은 암스테르담에 본사를 둔 Nasdaq 상장 AI 클라우드 기업입니다. 모델 학습부터 튜닝, 추론, 배포까지 이어지는 AI 인프라 플랫폼을 제공하며, GPU 클러스터와 글로벌 데이터센터 확장이 핵심 투자 포인트로 거론됩니다.",
    AAOI: "Applied Optoelectronics는 데이터센터, 케이블 broadband, FTTH 시장에 들어가는 광통신 제품을 설계·제조하는 Nasdaq 상장사입니다. 레이저 칩, 광부품, 트랜시버, HFC 네트워크 장비까지 수직 통합된 포트폴리오가 AI 데이터센터 네트워크 수요와 연결됩니다.",
    AEHR: "Aehr Test Systems는 반도체 웨이퍼 레벨 테스트와 burn-in 장비를 공급하는 Nasdaq 상장사입니다. SiC, GaN, 광포토닉스, 메모리 반도체의 신뢰성 테스트 수요가 커질 때 함께 언급되는 장비 업체입니다.",
    IREN: "IREN은 북미 전력 부지와 데이터센터를 기반으로 AI 클라우드, GPU 클러스터, 고성능 컴퓨팅 인프라를 제공하는 상장사입니다. 과거 비트코인 채굴 노출에서 AI 데이터센터와 재생에너지 기반 전력 인프라 테마로 재평가되는 흐름이 있습니다.",
    SPCX: "SpaceX는 2002년 설립된 민간 우주 기업으로 로켓과 우주선을 설계·제조·발사합니다. 재사용 로켓, 위성 인터넷 Starlink, 정부·상업 발사 수요가 핵심 사업이며, 사라했제에서는 공개시장 종목이 아니라 SpaceX 비상장 노출 또는 관련 발언을 별도 추적 자산으로 다룹니다.",
    ...
    SIVE: "Sivers Semiconductors는 무선, mmWave, 포토닉스 반도체 기술을 다루는 스웨덴 상장사입니다. AI 데이터센터, 통신, SATCOM을 잇는 고속 연결과 정밀 레이저 부품 수요가 투자 논리로 자주 연결됩니다.",
    XFAB: "X-FAB은 아날로그·mixed-signal 반도체에 특화된 pure-play 파운드리 그룹입니다. 자동차, 산업, 의료용 반도체와 CMOS, SOI, MEMS, SiC 공정 역량이 주요 관찰 포인트입니다.",
