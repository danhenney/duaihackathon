import { assets, people, seedCalls, viralPostFallback } from "../src/data.js";

export const priceFallbacks = {
  ...
  "000660.KS": { entry: 20000, current: 2521000, currency: "KRW" },
  ...
  AAOI: { entry: 170.81, current: 172.355, currency: "USD" },
  AEHR: { entry: 104.83, current: 115.56, currency: "USD" },
  NBIS: { entry: 265.1, current: 285.26, currency: "USD" },
  IREN: { entry: 59.18, current: 59.42, currency: "USD" },
  "005930.KS": { entry: 71300, current: 346500, currency: "KRW" },
...
export function searchLocal(query = "") {
  ...
  const matchesQuery = (value) => String(value || "").toLowerCase().includes(q);
...
export function normalizeModelSymbol(symbol = "") {
...
export function fallbackEntryPrice(symbol) {
...
export function chartParams(range = "all") {
  const map = {
...
export function downsamplePoints(points, maxPoints = 900) {
  if (points.length <= maxPoints) return points;
  const stride = Math.ceil(points.length / maxPoints);
  return points.filter((_, index) => index === 0 || index === points.length - 1 || index % stride === 0);
}

export function coinGeckoParams(range = "all") {
  const map = {
    "1m": "30",
    "6m": "180",
    "1y": "365",
...
export async function fetchCoinGeckoChart(symbol, range = "all") {
  const asset = assets[symbol];
  if (!asset?.coingecko) throw new Error("CoinGecko id missing");
  const days = coinGeckoParams(range);
  const url = `https://api.coingecko.com/api/v3/coins/${encodeURIComponent(asset.coingecko)}/market_chart?vs_currency=usd&days=${days}`;
...
export function rangeStartTime(range = "all") {
  const days = {
    "1m": 30,
    "6m": 180,
    "1y": 365
...
export async function fetchHyperliquidChart(symbol, range = "all") {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Hyperliquid chart failed: ${response.status}`);
    const payload = await response.json();
...
export async function fetchYahooChart(symbol, range = "all") {
  ...
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooSymbol)}?${query}&includePrePost=false`;
  const controller = new AbortController();
...
export async function tryDuckDuckGo(query) {
...
export function xUsernameFromQuery(query = "") {
...
export async function fetchXJson(url) {
...
export function tweetToCandidate(tweet, username) {
...
export function xSearchLimit(mode = "standard") {
  return mode === "deep" ? 100 : mode === "fast" ? 25 : 50;
}

export function investmentKeywordsFor(username = "", rawQuery = "") {
...
export async function fetchXUserProfile(query) {
...
export async function fetchXUserCandidates(query, mode = "standard") {
...
export async function fetchXKeywordCandidates(query, mode = "standard") {
  const profile = await fetchXUserProfile(query);
  if (!profile) return [];
...
export async function classifyCandidatesWithClaude(candidates) {
...
export function nearestClose(points, date) {
...
export async function attachPricesToCandidates(candidates) {
...
export async function resolveCandidatePrices(candidates) {
...
export { assets, people, seedCalls, viralPostFallback };
